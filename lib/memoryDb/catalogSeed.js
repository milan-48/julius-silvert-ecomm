...
export function buildCatalogProducts() {
  ...
  for (const cat of CATEGORIES) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const seed = i + slug.length + out.length;
      let stockCase = hasCasePc ? 6 + (seed * 5) % 28 : 0;
      let stockPc = hasCasePc ? 12 + (seed * 3) % 45 : 0;
      let stockSingle = hasCasePc ? 0 : 18 + (seed * 7) % 52;

      /** Default catalog data: first What’s New items demo low stock + CASE OOS (PC still available). */
      if (slug === "whats-new" && hasCasePc) {
        if (i === 0) {
          stockCase = 5; // ≤ LOW_STOCK_THRESHOLD → low stock on CASE
          stockPc = 36;
        } else if (i === 1) {
          stockCase = 0;
          stockPc = 28;
